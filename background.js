...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === 'updateExtensionData') {
        ...
        console.log("Обновление данных расширения в контент-скрипте");
...
function cvFile() {
    ...
    if (currentURL.includes('linkedin.com')) {
        ...
        // textArray = '<style>.normal-weight.text-muted-print{display: none;} .btn.btn-default.js-offer-job{display: none;} .hidden-print{display: none;} .dl-horizontal dt { float: left;} .flex-wrap { flex-wrap: wrap; } .flex { display: flex; } .skill { max-width: 600px; padding: 8px 15px; border-radius: 20px; color: #000; background-color: #f4f5f6; } .add-right-sm { margin-right: 10px!important; } .ellipsis { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .inline-block { display: inline-block; }</style><h1>'+ fullName.firstName + ' ' + fullName.lastName +'</h1>' +
        //     (h2Element ? '<h3>' + h2Element.innerHTML + '</h3>' : '') +
        //     (h3Element ? '<div>' + h3Element.innerHTML + '</div>' : '') +
        //     (pElement ? '<dl class="dl-horizontal">' + pElement.innerHTML + '</dl>' : '') +
        //     (h2Element3 ? '<h3>' + h2Element3.innerHTML + '</h3>' : '') +
        //     (h3Element3 ? '<div>' + h3Element3.innerHTML + '</div>' : '') +
        //     (pElement3 ? '<div>' + pElement3.innerHTML + '</div>' : '') +
        //     (h3Element33 ? '<div>' + h3Element33.innerHTML + '</div>' : '') +
        //     (pElement33 ? '<div>' + pElement33.innerHTML + '</div>' : '') +
        //     (h3Element34 ? '<div>' + h3Element34.innerHTML + '</div>' : '') +
        //     (pElement34 ? '<div>' + pElement34.innerHTML + '</div>' : '') +
        //     (h3Element35 ? '<div>' + h3Element35.innerHTML + '</div>' : '') +
        //     (pElement35 ? '<div>' + pElement35.innerHTML + '</div>' : '') +
        //     (h3Element36 ? '<div>' + h3Element36.innerHTML + '</div>' : '') +
        //     (pElement36 ? '<div>' + pElement36.innerHTML + '</div>' : '') +
        //     (h2Element4 ? '<h3>' + h2Element4.innerHTML + '</h3>' : '') +
        //     (h3Element4 ? '<div class="flex flex-wrap">' + h3Element4.innerHTML + '</div>' : '') +
        //     (pElement4 ? '<div>' + pElement4.innerHTML + '</div>' : '') +
        //     (h2Element5 ? '<h3>' + h2Element5.innerHTML + '</h3>' : '') +
        //     (h3Element5 ? '<div class="flex flex-wrap">' + h3Element5.innerHTML + '</div>' : '') +
        //     (pElement5 ? '<div>' + pElement5.innerHTML + '</div>' : '') +
        //     (h2Element6 ? '<h3>' + h2Element6.innerHTML + '</h3>' : '') +
        //     (h3Element6 ? '<div>' + h3Element6.innerHTML + '</div>' : '') +
        //     (pElement6 ? '<div>' + pElement6.innerHTML + '</div>' : '') +
        //     '<h3>Інформація</h3>' +
        //     (elWr ? '<div>' + elWr.innerHTML + '</div>' : '');
        ...
        chrome.runtime.sendMessage({action: 'checkCandidate', lastName: lastName}, function (response) {
            if (response && response.action === 'checkCandidateResult') {
                ...
                if(candidateInfoElement) { // Проверить существование элемента
                    candidateInfoElement.dataset.exists = response.exists ? response.exists : ''; // Установить значение атрибута data
                }
                resolve(response.exists);
            } else {
                console.error('Failed to check candidate');
